...
mod tests {
    ...
    #[test]
    fn recognizes_and_resolves_apple_music_urls() {
        assert_eq!(
            apple_source("https://music.apple.com/us/album/example/123?i=456"),
            Some(AppleSource::Song {
                storefront: "us".into(),
                id: "456".into()
            })
        );
        assert!(matches!(
            apple_source("https://music.apple.com/jp/song/example/123"),
            Some(AppleSource::Song { .. })
        ));
        assert!(!supports("https://example.com/us/album/example/123"));
    }

    #[test]
    fn locates_developer_jwts() {
        assert_eq!(
            jwt_in("prefix eyJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJhcHBsZSJ9.signature suffix"),
            Some("eyJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJhcHBsZSJ9.signature".into())
        );
    }

    #[test]
    fn finds_script_bundle_sources() {
        let sources: Vec<_> = script_sources("<script src=\"/assets/app.js\"></script>").collect();
        assert_eq!(sources, vec!["/assets/app.js"]);
    }

    #[test]
    fn key_endpoint_receives_adam_id_and_key_uri() {
        let url = template_url(
            "http://127.0.0.1:40020/key?existing=value",
            "1720704575",
            "skd://itunes.apple.com/p683167073/c23",
        )
        .expect("valid endpoint");
        let query: BTreeMap<_, _> = url.query_pairs().into_owned().collect();
        assert_eq!(query.get("existing"), Some(&"value".into()));
        assert_eq!(query.get("adamId"), Some(&"1720704575".into()));
        assert_eq!(
            query.get("uri"),
            Some(&"skd://itunes.apple.com/p683167073/c23".into())
        );
    }

    #[test]
    fn rejects_fragment_samples_outside_mdat() {
        let moof = Moof {
            traf: vec![mp4_atom::Traf {
                tfhd: mp4_atom::Tfhd {
                    default_sample_size: Some(16),
                    ..Default::default()
                },
        ...
        let mdat = FragmentAtom {
            kind: *b"mdat",
            start: 16,
            end: 32,
            header_len: 8,
        };
        assert!(fragment_samples(&moof, 0, &mdat).is_err());
    }
}
